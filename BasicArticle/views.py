		...
		if request.method == 'POST':
			...
			try:
				...
				try:
					...
					raise Http404
			response = render(request, 'edit_article.html', {'article': article, 'cmember':cmember,'gmember':gmember,'message':message, 'belongs_to':belongs_to,'transition': transition,'reject':reject, 'private':private,'url':settings.SERVERURL})
			return response
	else:
		return redirect('login')
